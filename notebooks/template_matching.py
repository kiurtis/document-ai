...
for file_name in image_files:
    ...
    if any(file_name.lower().endswith(ext) for ext in valid_image_extensions):
        ...
        try:
            ...
            print('-----------------')
            print("An error occurred trying to get bloc 2 and 4 of ", file_name ," :", e)
            print('-----------------')
        ...
        try:
            ...
            cropped_image_paths = [os.path.join(output_fold, f"{os.path.splitext(file_name)[0]}_{i}.jpeg") 
                                   for i in range(len(blocs))]
        except Exception as e:
            print('-----------------')
            print("An error occurred trying to crop the image ", file_name ," :", e)
            print('-----------------')

        #Dividing and cropping bloc 2:
        try:
            file_path_bloc2 = str(cropped_image_paths[0])
            bloc_2_info_path, bloc_2_sign_path = arval_classic_divide_and_crop_bloc2(file_path_bloc2,
                                                                                    output_fold,
                                                                                    file_name,
                                                                                    template_path_signature_bloc2)

            read_and_plot(bloc_2_info_path)
            print('  ')
            read_and_plot(bloc_2_sign_path)
        ...
        try:
            ...
            bloc_4_info_path,bloc_4_sign_path = arval_classic_divide_and_crop_bloc4(file_path_bloc4,
                                                                                    output_fold,
                                                                                    file_name,
                                                                                    template_path_signature_bloc4)

            read_and_plot(bloc_4_info_path)
            print('  ')
            read_and_plot(bloc_4_sign_path)
