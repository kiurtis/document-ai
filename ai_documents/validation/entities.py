...
class ResultValidator:
    def __init__(self, results, plate_number, valet_name=None, from_concessionaire=None, to_concessionaire=None):
        ...
        self.quality_is_ok = False # Only for ArvalClassicGPTDocumentAnalyzer
        self.signatures_are_ok = False
        self.stamps_are_ok = False
        self.mileage_is_filled = False
        self.number_plate_is_filled = False
        self.number_plate_is_right = False
        self.block4_is_filled = False
        self.block4_is_filled_by_company = False
        self.restitution_date_is_filled = False
        self.serial_number_is_filled = False
    ...
    def validate_quality(self):
        self.quality_is_ok = self.results['overall_quality'].lower() == 'yes'

    def validate_signatures(self):
        signature_block_2_condition = self.results['signature_and_stamp_block_2'] in ('both', 'signature')
        signature_block_4_condition = self.results['signature_and_stamp_block_4'] in ('both', 'signature')
    ...
    def validate_stamps(self):
        stamp_block_2_condition = self.results['signature_and_stamp_block_2'] in ('both', 'stamp') or (self.from_concessionaire is False) # If not from concessionaire, no stamp needed
        stamp_block_4_condition = self.results['signature_and_stamp_block_4'] in ('both', 'stamp') or (self.to_concessionaire is False) # If not to concessionaire, no stamp needed
    ...
    def validate_mileage(self):
        self.mileage_is_filled = self.results['block_2']['Kilométrage'] not in ["<EMPTY>", "<NOT_FOUND>"]

    def validate_restitution_date(self):
        self.restitution_date_is_filled = self.results['block_2']['Restitué le'] not in ["<EMPTY>", "<NOT_FOUND>"]

    def validate_serial_number(self):
        self.serial_number_is_filled = self.results['block_2']['N° de série'] not in ["<EMPTY>", "<NOT_FOUND>"]

    def validate_number_plate_is_filled(self):
        self.number_plate_is_filled = self.results['block_2']['Immatriculé'] not in ["<EMPTY>", "<NOT_FOUND>"]

    def validate_number_plate_is_right(self):
        detected_plate_number = self.results['block_2']['Immatriculé']
    ...
    def validate_block4_is_filled_by_company(self, distance_margin=4):
        company_name = self.results['block_4']['Société']
        self.block4_is_filled_by_company = l_distance(company_name, "Pop Valet") > distance_margin
        if self.valet_name is not None:
            self.block4_is_filled_by_company = self.block4_is_filled_by_company and (self.driver_name_is_filled != self.valet_name)

    def validate_block4_is_filled(self):
        self.driver_name_is_filled = self.results['block_4']['Nom et prénom'] not in ["<EMPTY>", "<NOT_FOUND>"]
        self.telephone_is_filled = self.results['block_4']['Tél'] not in ["<EMPTY>", "<NOT_FOUND>"]
        self.email_is_filled = self.results['block_4']['E-mail'] not in ["<EMPTY>", "<NOT_FOUND>"]

        if self.to_concessionaire:
            concessionaire_condition = self.results['block_4']['Société'] not in ["<EMPTY>", "<NOT_FOUND>"]
        else:
            concessionaire_condition = True

        self.block4_is_filled = (self.telephone_is_filled or self.email_is_filled) and self.driver_name_is_filled and \
    ...
    def gather_refused_motivs(self):
        ...
        if not self.serial_number_is_filled:
            self.refused_causes.append('serial_number_is_not_filled')
        if not self.telephone_is_filled:
            self.refused_causes.append('telephone_is_not_filled')
        if not self.email_is_filled:
        ...
        if not self.restitution_date_is_filled:
            self.refused_causes.append('restitution_date_is_not_filled')
    def validate(self):
        try:
